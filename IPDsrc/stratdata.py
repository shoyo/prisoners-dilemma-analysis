...
class Tester(TitForTat):
    ...
    alternating between defecting and cooperating. (TF2T exploiter)"""
    ...
    def decide_action(self):
        self.turn += 1
        if self.testing_turn == 0 and not randint(0, 5):  # testing
            self.testing_turn += 1
            return False
        elif 0 < self.testing_turn <= 1:  # turn of cooperation
            self.testing_turn += 1
...
class Conniver(TitForTat):
    ...
    2 turns of it defecting, it continues defecting. (smart Kantian exploiter)"""
